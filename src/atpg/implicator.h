...
public: 
    ...
    void GetPiPattern(Pattern *p, vector<int>* maval = 0) const; 
...
inline void Implicator::GetPiPattern(Pattern *p, vector<int>* maval) const {
    for (int i=0; i<cir_->npi_; i++){
        p->pi1_[i] = Get3Val(i); 
        if(maval != 0){
            maval->at(cir_->orderIdtoUnorderedId.at(i) + 1) = p->pi1_[i];
        }
    }
    return;  
