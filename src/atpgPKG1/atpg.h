...
namespace CoreNs
{
	...
	public:
		...
		// my method
		void runSingleSAF(PatternProcessor *pPatternProcessor, int gateID, CoreNs::Fault::FAULT_TYPE faultType); // run single stuck at fault
	...
	// inline Atpg::Atpg(Circuit *pCircuit)
			: pCircuit_(pCircuit),
	...
	inline Value Atpg::evaluateGoodVal(Gate &gate)
	{
		...
		switch (gate.gateType_)
		{
			...
			case Gate::AND2:
				// my content // deal with input complement
				if(gate.phaseFanins[0] == 1)	v[0] = cINV(v[0]);
				if(gate.phaseFanins[1] == 1)	v[1] = cINV(v[1]);
	...
	inline Value Atpg::evaluateFaultyVal(Gate &gate)
	{
		...
		switch (gate.gateType_)
		{
			...
			case Gate::BUF:
				...
				// my content // deal with input complement
				return gate.phaseFanins[0] == 1 ? cINV(val) : val;
			case Gate::INV:
				val = pCircuit_->circuitGates_[gate.faninVector_[0]].atpgVal_;
				if (faultyLine == 0)
				{
			...
			case Gate::AND4:
				...
				else
				{
					...
					val = gate.phaseFanins[faultyLine - 1] == 1 ? cINV(val) : val;
