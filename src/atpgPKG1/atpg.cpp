...
void Atpg::runSingleSAF(PatternProcessor *pPatternProcessor, int gateID, CoreNs::Fault::FAULT_TYPE faultType){
	Fault *pCurrentFault = NULL;
	FaultPtrList originalFaultPtrList;//, faultPtrListForSTC;
	setupCircuitParameter();
	// open later
	...
	// setting faults for running ATPG
	CoreNs::Fault* targetFault = new CoreNs::Fault(gateID, faultType, 0); // 要加上input 的 (gateId, faulttype , 1 )
	originalFaultPtrList.push_back(targetFault);
	// faultPtrListForSTC.push_back(targetFault);
...
void Atpg::calculateGateDepthFromPO()
{
	Abc_Obj_t* pObj;
	int i;
	Abc_NtkForEachPo(pCircuit_->pNtk_, pObj, i){
		// pCircuit_->circuitGates_[pObj->Id].depthFromPo_ = 0;
		pCircuit_->circuitGates_[myAbc_ObjId(pObj)].depthFromPo_ = 0;
	}
	pCircuit_->circuitGates_[0].depthFromPo_ = 0;
	Abc_NtkForEachObjReverse(pCircuit_->pNtk_, pObj, i){
		// Gate &gate = pCircuit_->circuitGates_[pObj->Id];
		Gate &gate = pCircuit_->circuitGates_[myAbc_ObjId(pObj)];
		// gateID_to_valModified_[pObj->Id] = 0; // sneak the initialization assignment in here
		gateID_to_valModified_[myAbc_ObjId(pObj)] = 0; // sneak the initialization assignment in here
		if(pObj)
		if (gate.gateType_ == Gate::PO || ABC_OBJ_CONST1 == Abc_ObjType(pObj))
			continue;
	...
	// Abc_NtkForEachObjReverse(pCircuit_->pNtk_, pObj, i){
	// 	std::cout <<"Gate " << pObj->Id << "'s depthFromPo_ = " << pCircuit_->circuitGates_[pObj->Id].depthFromPo_ << std::endl;
	// 	std::cout <<"Gate " << pObj->Id << "'s gate.numFO_ = " << pCircuit_->circuitGates_[pObj->Id].numFO_ << std::endl;
	// 	std::cout << "fanout = ";
	// 	for(auto gt : pCircuit_->circuitGates_[pObj->Id].fanoutVector_){
	// 		std::cout << pCircuit_->circuitGates_[gt].gateId_ << ' ';
	// 	}
	// 	std::cout << "fanin = ";
	// 	for(auto gt : pCircuit_->circuitGates_[pObj->Id].faninVector_){
	// 		std::cout << pCircuit_->circuitGates_[gt].gateId_ << ' ';
	// 	}
	// 	std::cout << std::endl;
	// 	std::cout << "\n===================================\n" << std::endl;
...
void Atpg::identifyGateDominator()
{
	...
	Abc_NtkForEachObjReverse(pCircuit_->pNtk_, pObj, i)
	{
		...
		// int gateCount = pushGateFanoutsToEventStack(pObj->Id);
		int gateCount = pushGateFanoutsToEventStack(myAbc_ObjId(pObj));
...
void Atpg::identifyGateUniquePath()
{
	...
	Abc_NtkForEachObjReverse(pCircuit_->pNtk_, pObj, i)
	{
		...
		// reachableByDominator[gate.gateId_] = pObj->Id;
		reachableByDominator[gate.gateId_] = myAbc_ObjId(pObj);
		// int count = pushGateFanoutsToEventStack(pObj->Id);
		int count = pushGateFanoutsToEventStack(myAbc_ObjId(pObj));
		for (int j = gate.numLevel_ + 1; j < pCircuit_->totalLvl_; ++j)
		{
			while (!circuitLevel_to_EventStack_[j].empty())
			{ // if fanout gate was not empty
				...
				// reachableByDominator[gTmp.gateId_] = pObj->Id;
				reachableByDominator[gTmp.gateId_] = myAbc_ObjId(pObj);
				--count;
				if (count == 0)
				{
					for (int gReachID : gTmp.faninVector_)
					{
						// if (reachableByDominator[gReachID] == pObj->Id) // if it is UniquePath
						if (reachableByDominator[gReachID] == myAbc_ObjId(pObj)) // if it is UniquePath
						{
	...
	if (result == PATTERN_FOUND)
	{
		...
		if (pPatternProcessor->dynamicCompression_ == PatternProcessor::ON)
		{
			...
			for (Fault *pFault : faultListTemp)
			{
				...
				Gate *pGateForActivation = getGateForFaultActivation(*pFault);	// get fanin cone
...
Atpg::IMPLICATION_STATUS Atpg::doOneGateBackwardImplication(Gate *pGate)
{
	...
	if (pGate->gateType_ == Gate::BUF || pGate->gateType_ == Gate::INV || pGate->gateType_ == Gate::PO || pGate->gateType_ == Gate::PPO)
	{
		...
		pImpGate->atpgVal_ = pGate->phaseFanins[0] == 1 ? cINV(pImpGate->atpgVal_) : pImpGate->atpgVal_;
	...
	else
	{
		...
		if (pGate->atpgVal_ == OutputControlVal)
		{
			...
			for (int i = 0; i < pGate->numFI_; ++i)
			{
				...
				{
                    // my content // deal with input complement
					Value InputNonControlVali = pGate->phaseFanins[i] == 1 ? cINV(InputNonControlVal) : InputNonControlVal;
		...
		else
		{
			...
			if (NumOfX == 1)
			{
				Gate *pImpGate = &pCircuit_->circuitGates_[pGate->faninVector_[ImpPtr]];
				// my content // deal with input complement
				Value InputControlVali = pGate->phaseFanins[ImpPtr] == 1 ? cINV(InputControlVal) : InputControlVal;
				pImpGate->atpgVal_ = InputControlVali;
				gateID_to_valModified_[pGate->gateId_] = 1;
				backtrackImplicatedGateIDs_.push_back(pImpGate->gateId_);
				pushGateToEventStack(pGate->faninVector_[ImpPtr]);
				pushGateFanoutsToEventStack(pGate->faninVector_[ImpPtr]);
	...
	if (pFaultyGate->gateId_ != gate.gateId_)
	{
		...
		if (NonControlVal != X)
		{
			for (int i = 0; i < gate.numFI_; ++i)
			{
				...
				Value NonControlVali = gate.phaseFanins[i] == 1 ? cINV(NonControlVal) : NonControlVal;
				if (pFaninGate->atpgVal_ == X)
				{
					pFaninGate->atpgVal_ = NonControlVali;
				...
				else if (pFaninGate->atpgVal_ == NonControlVali) // my content // deal with input complement
				{
					return UNIQUE_PATH_SENSITIZE_FAIL;
				}
			}
		...
		{ // If pNextGate is Unary and its NonControlVal is not unknown.
			if (pCurrGate->numFO_ == 1)
			{
				...
				for (int i = 0; i < pNextGate->numFI_; ++i)
				{
					...
					Value NonControlVali = pNextGate->phaseFanins[i] == 1 ? cINV(NonControlVal) : NonControlVal;
                    // my content // deal with input complement
					// if (pFaninGate != pCurrGate && pNextGate->getInputCtrlValue() != X && pFaninGate->atpgVal_ == pNextGate->getInputCtrlValue())
					if (pFaninGate != pCurrGate && NonControlVali != X && pFaninGate->atpgVal_ == NonControlVali)
					{
						return UNIQUE_PATH_SENSITIZE_FAIL;
					}
					...
					if (pFaninGate != pCurrGate && pFaninGate->atpgVal_ == X)
					{
						...
						pFaninGate->atpgVal_ = NonControlVali; // Set input gate of pNextGate to pNextGate's NonControlVal
			...
			else
			{
				...
				for (int i = 0; i < pNextGate->numFI_; ++i)
				{
					...
					if (!DependOnCurrent)
					{
						...
						if (pFaninGate->atpgVal_ != X && pFaninGate->atpgVal_ == NonControlVali && NonControlVali != X)
						{
							return UNIQUE_PATH_SENSITIZE_FAIL;
						}
...
int Atpg::setFaultyGate(Fault &fault)
{
	...
	if (!isOutputFault)
	{
		...
		if (pFaultyGate->gateType_ == Gate::AND2 || pFaultyGate->gateType_ == Gate::AND3 || pFaultyGate->gateType_ == Gate::AND4 || pFaultyGate->gateType_ == Gate::NAND2 || pFaultyGate->gateType_ == Gate::NAND3 || pFaultyGate->gateType_ == Gate::NAND4 || pFaultyGate->gateType_ == Gate::NOR2 || pFaultyGate->gateType_ == Gate::NOR3 || pFaultyGate->gateType_ == Gate::NOR4 || pFaultyGate->gateType_ == Gate::OR2 || pFaultyGate->gateType_ == Gate::OR3 || pFaultyGate->gateType_ == Gate::OR4)
		{
			...
			for (int i = 0; i < pFaultyGate->numFI_; ++i)
			{
				...
				{	
					// my content // deal with input complement
					Value desiredInputNonCtrlValue = pFaultyGate->phaseFanins[i] ? cINV(pFaultyGate->getInputNonCtrlValue()) : pFaultyGate->getInputNonCtrlValue();
					if (pFaninGate->atpgVal_ == X)
					{
						// pFaninGate->atpgVal_ = pFaultyGate->getInputNonCtrlValue();
						// 必and2 --> 看input腳有沒有complement
						pFaninGate->atpgVal_ = desiredInputNonCtrlValue;
						backtrackImplicatedGateIDs_.push_back(pFaninGate->gateId_);
					}
					// else if (pFaninGate->atpgVal_ != pFaultyGate->getInputNonCtrlValue())
					else if (pFaninGate->atpgVal_ != desiredInputNonCtrlValue)
					{
		...
		else if (pFaultyGate->gateType_ == Gate::INV || pFaultyGate->gateType_ == Gate::BUF || pFaultyGate->gateType_ == Gate::PO || pFaultyGate->gateType_ == Gate::PPO)
		{
			...
			// valueTemp = pFaultyGate->isInverse();
			// pFaultyGate->atpgVal_ = cXOR2(valueTemp, FaultyValue);
			pFaultyGate->atpgVal_ = cXOR2(pFaultyGate->phaseFanins[0] == 1 ? H : L, FaultyValue);
			backtrackImplicatedGateIDs_.push_back(pFaultyGate->gateId_);
		}
	...
	{ // output fault
		...
		else if (pFaultyGate->gateType_ == Gate::INV || pFaultyGate->gateType_ == Gate::BUF || pFaultyGate->gateType_ == Gate::PO || pFaultyGate->gateType_ == Gate::PPO)
		{
			...
			// my content // deal with input complement
			pFaultyGate->atpgVal_ = cXOR2(pFaultyGate->phaseFanins[0] == 1 ? H : L, Val);
			backtrackImplicatedGateIDs_.push_back(pFaninGate->gateId_);
		...
		switch (atpgStatus)
		{
			...
			case CURRENT_OBJ_DETERMINE:
				...
				{ // NO
					...
					// my content // deal with input complement // 沒改，但應該不用改 // 等等下面每個fanin那邊再改？
					...
					for (int i = 0; i < pCurrentObj->numFI_; ++i)
					{
						...
						{ // YES
							// rule 1 ~ rule 5
							// my content // deal with input complement
							Value Vali = pCurrentObj->phaseFanins[i] == 1 ? cINV(Val) : Val;
							if (pFaninGate == pEasiestInput)
							{
								if(pCurrentObj->phaseFanins[i] == 0){
									nn0 = n0;
									nn1 = n1;
								}
								else{
							...
							else
							{
								...
								if(pCurrentObj->phaseFanins[i] == 1){
									int tmp = nn0;
									nn0 = nn1;
									nn1 = nn0;
								}
	...
	for (const int &currentObjectGateID : currentObjectives_)
	{
		...
		{ // if single value of the gate is X or Z or I, set line number depend on gate type
			switch (pGate->gateType_)
			{
				...
				case Gate::XNOR2:
				case Gate::XNOR3:
				// my content // deal with input complement // 沒改，可能要改
					setGaten0n1(pGate->gateId_, 0, 1);
					break;
				case Gate::OR2:
				case Gate::OR3:
