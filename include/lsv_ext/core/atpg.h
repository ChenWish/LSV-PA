...
namespace CoreNs
{
	...
	inline Value Atpg::evaluateGoodVal(Gate &gate)
	{
		...
		switch (gate.gateType_)
		{
			...
			case Gate::PPO:
			case Gate::BUF:
				// my content // deal with input complement
				return gate.phaseFanins[0] == 1 ? cINV(v[0]) : v[0];
			case Gate::INV:
				return cINV(v[0]);
			case Gate::AND2:
				// my content // deal with input complement
				v[0] = gate.phaseFanins[0] == 1 ? cINV(v[0]) : v[0];
				v[1] = gate.phaseFanins[1] == 1 ? cINV(v[1]) : v[1];
	...
	inline Value Atpg::evaluateFaultyVal(Gate &gate)
	{
		...
		switch (gate.gateType_)
		{
			...
			case Gate::BUF:
				...
				return pCircuit_->circuitGates_[gate.faninVector_[0]].phaseFanins[0] == 1 ? cINV(val) : val;
			case Gate::INV:
				val = pCircuit_->circuitGates_[gate.faninVector_[0]].atpgVal_;
				if (faultyLine == 0)
				{
			...
			case Gate::AND4:
				if (faultyLine == 0)
				{
					// my content // deal with input complement
					val = pCircuit_->circuitGates_[gate.faninVector_[0]].atpgVal_;
					val = pCircuit_->circuitGates_[gate.faninVector_[0]].phaseFanins[0] == 1 ? cINV(val) : val;
					
					for (int i = 1; i < gate.numFI_; ++i)	// numFI_ should = 2
					{
						Value tmp = pCircuit_->circuitGates_[gate.faninVector_[i]].atpgVal_;
						tmp = pCircuit_->circuitGates_[gate.faninVector_[i]].phaseFanins[1] == 1 ? cINV(tmp) : tmp;
				...
				else
				{
					...
					val = pCircuit_->circuitGates_[gate.faninVector_[faultyLine - 1]].phaseFanins[0] == 1 ? cINV(val) : val;
					...
					for (int i = 0; i < gate.numFI_; ++i)
					{
						if (i != faultyLine - 1)
						{
							// my content // deal with input complement
							Value tmp = pCircuit_->circuitGates_[gate.faninVector_[i]].atpgVal_;
							tmp = pCircuit_->circuitGates_[gate.faninVector_[i]].phaseFanins[i] == 1 ? cINV(tmp) : tmp;
							// val = cAND2(val, pCircuit_->circuitGates_[gate.faninVector_[i]].atpgVal_);
							val = cAND2(val, tmp);
						}
					}
